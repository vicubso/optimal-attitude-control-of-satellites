...
%path = '/home/vicubso/Dropbox/TFG_MEN/Results/New/sing_rest_rest_nonlinear_limit1_q0_r0_T2-52/';
...
set(gcf, 'resize', 'off')
plot(time,angveloc,'-','linewidth',2)
...
%text(1.55,0.075,'\omega_1=\omega_2=0')
...
set(gcf, 'resize', 'off')
hold on
%plot([0,time(end)],[1,1],'k--','linewidth',1.5,'HandleVisibility','off')
%plot([0,time(end)],[-1,-1],'k--','linewidth',1.5,'HandleVisibility','off')
plot(time(1:end-1),control,'-','linewidth',2)
hold off
xlim([time(1),time(end-1)])
ylim([min(min(control))-0.2,max(max(control))+0.2])
grid on
grid minor
xlabel('t (s)')
ylabel('u (Nm)')
legend('u_1','u_2','u_3')
%text(1.55,0.09,'u_1=u_2=0')
...
set(gca,'XMinorTick','on','YMinorTick','on')
set(gcf, 'resize', 'off')
plot(time,eulerang,'-','linewidth',2)
xlim([time(1),time(end)])
xlim([time(1),time(end)])
ylim([min(min(eulerang))-pi/8,max(max(eulerang))+pi/4])
set(gca,'YTick',-pi:pi/8:pi) 
set(gca,'YTickLabel',pi_ticks)
grid on
grid minor
...
xlim([time(1),time(end)])
ylim([min(min(quaternion))-0.2,max(max(quaternion))+0.7])
grid on
grid minor
xlabel('t (s)')
ylabel('q')
...
set(gca,'XMinorTick','on','YMinorTick','on')
set(gcf, 'resize', 'off')
plot(time,real_angveloc,'-','linewidth',2)
xlim([time(1),time(end)])
...
set(gca,'XMinorTick','on','YMinorTick','on')
set(gcf, 'resize', 'off')
plot(time,real_eulerang,'-','linewidth',2)
xlim([time(1),time(end)])
xlim([time(1),time(end)])
ylim([min(min(real_eulerang))-pi/8,max(max(real_eulerang))+pi/4])
set(gca,'YTick',-pi:pi/8:pi) 
set(gca,'YTickLabel',pi_ticks)
grid on
grid minor
...
xlim([time(1),time(end)])
ylim([min(min(real_quaternion))-0.2,max(max(real_quaternion))+0.7])
grid on
grid minor
xlabel('t (s)')
ylabel('q')
